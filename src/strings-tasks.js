...
function removeFirstOccurrences(str, value) {
  const delidx = str.indexOf(value);
  if (delidx === -1) {
    return str;
  }
  return str.slice(0, delidx) + str.slice(delidx + value.length);
}
...
function removeLastOccurrences(str, value) {
  const delidx = str.lastIndexOf(value);
  if (delidx === -1) {
    return str;
  }
  return str.slice(0, delidx) + str.slice(delidx + value.length);
...
function formatTime(minutes, seconds) {
  ...
  return minutes1
    .padStart(2, '0')
    .concat(':'.concat(seconds1.padStart(2, '0')));
}
...
function isPalindrome(str) {
  const subs = str
    ...
    .replaceAll('\t', '')
    .replaceAll('?', '');
  return subs === subs.split('').reverse().join('');
}
...
function invertCase(str) {
  ...
  let res = '';
  for (let i = 0; i <= str.length - 1; i += 1) {
    if (bigstr.includes(str[i])) {
      res = res.concat(str[i].toLowerCase());
    } else if (smallstr.includes(str[i])) {
      res = res.concat(str[i].toUpperCase());
    } else res = res.concat(str[i]);
  }
  return res;
}
...
function extractNameFromTemplate(value) {
  const regex = / .*/;
  const s = value.match(regex);
  return s[0].trim().replace('!', '');
}
...
function extractEmails(str) {
  ...
  for (let i = 0; i <= strlist.length - 1; i += 1) {
    emaillist.push(
      strlist[i].match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/gi)[0]
    );
  }
  return emaillist;
}
